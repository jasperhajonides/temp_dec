...
def bias_fitting_single_timepoint(tuning,distractor,split,nr_bins=64):
    ...
    ----------
    tuning     : ndarray
            3d array of tuning curve bins by repeats by time
    distractor : ndarray
            vector of tuning curve phase ranging from [-pi-x to pi]
            where x is the the stepsize
    splits     : ndarray
    ...
    pe = np.zeros((nr_bins,tuning.shape[2]))
    phase_conditions = np.zeros((tuning.shape[2],int(split.max()),))*np.nan
    ...
    if len(split) == tuning.shape[1]:
        for con in range(1,int(split.max()+1)): 
            print('%s. '%con + '%s' %len(np.unique(distr_digitize[split[~np.isnan(distractor)]==con])) + '/%s' %nr_bins +  ' unique target-distractor bins')
    ...
    for tp in range (tuning.shape[2]):
        ...
        if len(split) == tuning.shape[1]:
            for con in range(1,int(split.max()+1)):
                phase_conditions[tp,con-1] = output_fit['phase'][split[~np.isnan(distractor)]==con].mean()
...
def bias_fitting_difference(tuning):
    """Fit a cosine tuning curve and determine the phase shift of the present information
    towards or away a second orientation
    ...
    ----------
    tuning     : ndarray
            3d array of tuning curve conditions by bins by time
    ...
    phase_conditions = np.zeros((tuning.shape[0],tuning.shape[2]))
    amplitude_conditions = np.zeros((tuning.shape[0],tuning.shape[2]))

    for tp in range (tuning.shape[2]):

        output_fit = cosine_least_squares_fit(tuning[:,:,tp].T)
        phase_conditions[:,tp] = output_fit['phase']
        amplitude_conditions[:,tp] = output_fit['amplitude']
        

    output_fit['phase_conditions'] = phase_conditions
    output_fit['amplitude_conditions'] = amplitude_conditions
